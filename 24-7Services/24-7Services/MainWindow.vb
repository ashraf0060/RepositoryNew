...
Public Class MainWindow
    ...
    Private Sub MainWindow_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        CombMin.Text = Now.Minute - 2
        AddHandler CombMin.SelectedIndexChanged, AddressOf CombMin_SelectedIndexChanged
...
#End Region
    ...
    Private Sub EscSub()
                    ...
                    For Cnt = 0 To EscAtoTable.Rows.Count - 1
                        ...
                        If EscAtoTable.Rows(Cnt).Item("TkupEvtId") = 902 Then
                            ...
                            If EscAtoTable.Rows(Cnt).Item("TkupSendEsc") = False Then
                                If Format(ServrTime(), "HH:mm:ss") < #1/1/0001 04:00:00 PM# AndAlso Format(ServrTime(), "HH:mm:ss") > #1/1/0001 09:00:00 AM# AndAlso WdysTable.Rows(0).Item("HDy") = 1 Then
        ...
        Span_ = TimeSpan.Parse("00:01:00")
        nxt = " On " & Now.Add(TimeSpan.Parse("00:01:00"))
        RemoveHandler CombMin.SelectedIndexChanged, AddressOf CombMin_SelectedIndexChanged
        Invoke(Sub() CombHour.Text = Now.Hour)
        Invoke(Sub() CombMin.Text = Now.Minute - 2)
        AddHandler CombMin.SelectedIndexChanged, AddressOf CombMin_SelectedIndexChanged
...
Done_:
    ...
    Private Sub NTF()
        ...
        Invoke(Sub() CombMin.Enabled = True)
        Invoke(Sub() TimerEsc.Start())
        Invoke(Sub() TimerSecnods.Start())
    End Sub
    Private Sub CombMin_SelectedIndexChanged(sender As Object, e As EventArgs)
        CombMin.Enabled = False
        CombHour.Enabled = False
        TimerEsc.Stop()
        TimerSecnods.Stop()
        Dim NTFlTsk As New Thread(AddressOf NTF)
        NTFlTsk.IsBackground = True
        TimerEsc.Stop()
        TimerSecnods.Stop()
        NTFlTsk.Start()
    End Sub
