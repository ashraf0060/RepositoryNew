Public Class FolwTeam
    ...
    Private Sub FolwTicket_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Else
            ...
            If Mid(Usr.PUsrLvl, 17, 1) = "A" Then
                UserTree.Nodes.Add("5", 22 & " - " & "الإدارة العامة لخدمة العملاء" & " - " & "رفعت السعيد", 1, 3)
            Else
                UserTree.Nodes.Add(Usr.PUsrCat.ToString, Usr.PUsrID & " - " & Usr.PUsrCatNm & " - " & Usr.PUsrRlNm, 1, 3)
            End If
            '                   0  ,    1  ,     2    ,    3   ,     4     as mix name                 ***   
            If GetTbl("Select UsrId, UCatId, UCatIdSub, UCatLvl, UCatNm + N' - ' + UsrRealNm AS UsrMix From Int_user RIGHT OUTER Join IntUserCat On UsrCat = UCatId Where (UsrSusp = 0) AND (UCatLvl between 3 and 5) Order By UCatIdSub, UsrRealNm", UsrTable, "1025&H") = Nothing Then
    ...
    Private Sub Filtr()
        ...
        If IsNothing(UserTree.SelectedNode) = False Then
            If FltrStr.Length > 0 Then
                FltrStr &= " And " & Mid(UsrStr, 5, UsrStr.Length)
            Else
                FltrStr = Mid(UsrStr, 5, UsrStr.Length)
            End If
        End If
    ...
    Private Sub GridUpdt_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridUpdt.CellClick
        If GridUpdt.CurrentRow.Cells(6).Value = False Then
            If PublicCode.InsUpd("update TkEvent set TkupUnread = 1, TkupReDt = " & "CONVERT(DATETIME, '" & ServrTime() & "',103) where TkupSQL = " & GridUpdt.CurrentRow.Cells(0).Value & ";", "1035&H") = Nothing Then
    ...
    Private Sub BtnClos_Click(sender As Object, e As EventArgs) Handles BtnClos.Click
        ...
        If Rslt = DialogResult.Yes Then
            BtnClos.Enabled = False
            If InsTrans("update Tickets set TkDtClose = '" & Format(ServrTime(), "yyyy-MM-dd HH:mm:ss") & "', TkDuration = " & CalDate(StruGrdTk.DtStrt, CStr(Today), "1036&H") & ", TkClsStatus = 1" & ", TkFolw = 1" & " where (TkSQL = " & StruGrdTk.Sql & ");",
                ...
                MsgInf("تم إغلاق الشكوى رقم " & StruGrdTk.TkId & " في عدد " & CalDate(StruGrdTk.DtStrt, CStr(Today), "1036&H") & " يوم عمل")
            Else
                BtnClos.Enabled = True
                MsgErr(My.Resources.ConnErr & vbCrLf & My.Resources.TryAgain)
            End If
    ...
    Private Sub UserTree_AfterSelect(sender As Object, e As TreeViewEventArgs) Handles UserTree.AfterSelect
        If UserTree.SelectedNode.Nodes.Count > 0 Then
            ...
            End If
        Else
            UsrStr = "AND (TkEmpNm = " & Split(UserTree.SelectedNode.Text, "-")(0) & ")"
        End If
        Filtr()
    End Sub
